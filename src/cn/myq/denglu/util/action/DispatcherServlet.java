package cn.myq.denglu.util.action;
...
import cn.myq.denglu.util.BaseValidate;
import cn.myq.denglu.util.BeanOperate;
import cn.myq.denglu.util.BeanValidate;
import cn.myq.denglu.util.StringUtils;
...
public abstract class DispatcherServlet extends HttpServlet {
	...
	public static final String DEFAULT_FILENAME = "nophoto.jpg";
	...
	public boolean validateCode() {
		...
		return BaseValidate.validateCompareIgnore(code, rand) ;
	}
	 ...
	 * @param fileName
	 */
	public final void saveFile(String fileName) throws Exception {
		if (fileName == null) {
			return;
		}
	 ...
	 * @throws Exception
	 */
	public final void saveFile(List<String> fileNames) throws Exception {
		if (fileNames == null) {
			return;
		}
	 ...
	 * @throws Exception
	 */
	public final void deleteFile(Set<String> fileNames) throws Exception {
	 ...
	 * @return
	 */
	public final String getParamValue(String paramName) {
	 ...
	 * @return
	 */
	public final String[] getParamValues(String paramName) {
		String contentType = this.request.getContentType();
		if (contentType == null) {
			contentType = "" ;
		}
	 ...
	 * @return
	 */
	public final boolean validate(String property) {
	 ...
	 */
	public final void handleParameter() { // 处理所有的参数
		// application/x-www-form-urlencoded
		String contentType = this.request.getContentType();
		if (contentType == null) {
			contentType = "" ;
		}
		// System.out.println("************* name =  " + request.getParameter("product.pid"));
		...
		while (enu.hasMoreElements()) {
			...
			if (paramName.contains(".")) { // 现在是需要用户自动设置内容
				String voName = paramName.substring(0, paramName.indexOf("."));
				...
				try {
					...
					if (bo.getField().getType().getSimpleName().contains("[]")) { // 表示数组
	 ...
	 * 此方法负责处理所有的分页操作
	 */
	public final void handleSplitPage() {
		try {
	 ...
	 *            要提示的用户信息
	 */
	public final void setPathAndMsg(String pathKey, String msgKey) {
	...
	/**
	 * 如果现在没有文件上传，那么使用原始的图片名称，如果有文件上传，
	 * 那么没有名字，重新生成一个新的，如果有名字继续使用已有的
	 * @return
	 */
	public String getOldSingleFileName(String oldName) {
		try {
			if (this.smart == null || this.smart.getFiles().getSize() == 0) {
				return oldName;
			}
		...
		if (DEFAULT_FILENAME.equalsIgnoreCase(oldName)) {	// 图片名称为
			return this.createSingleFileName() ;
		}
		return oldName ;
	}
	...
	public void sendParameter(String paramName,String paramValue) {
