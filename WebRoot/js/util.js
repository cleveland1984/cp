...
function deleteForm(formId,eleId,url) {
	if (window.confirm("确定要删除数据吗？")) {
		var checkElements = document.all(eleId) ;	// 取得所有的复选框内容
		var flag = false ;
		// 保证有选项被选中之后才执行删除的操作
		if (checkElements.length == undefined) {	// 表示不是一个数组
			if (checkedElements.checked == true) {
				flag = true ;	// 可以执行删除操作
			}
		} else {
			for (var x = 0 ; x < checkElements.length ; x ++) {
...
// 修改商品的购买数量
function changeAmount(id,num) {
	var amount = document.getElementById(id) ;
	var val = parseInt(amount.value) + num ;
	if (val > 0) {
		amount.value = val ;
	} else {
		amount.value = 0 ;
...
function deleteBatch(url,eleId) {
	...
	if (checkElements.length == undefined) {	// 表示不是一个数组
		if (checkElements.checked == false) {
			...
			flag = true ;	// 可以执行删除操作
			all[0] = checkElements.value ;	// 取出内容
		}
	} else {
		var foot = 0 ;
		for (var x = 0 ; x < checkElements.length ; x ++) {
	...
	if (flag) {	// 此时包含有删除的数据项
		if (window.confirm("确定要删除这些数据吗？")) {
			if (all.length == 1) {	// 现在只有一个内容
				window.location = url + "?"+eleId+"=" + all[0] ;
			} else {	// 按照数组处理
				var param = "" ;	// 保存删除的数据
